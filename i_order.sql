...
BEGIN
	...
	if(select count(1) from customer where customer_first_name=@customer_fname and customer_last_name=@customer_lname and Phone=@customer_mobile) IS NULL
		select @Ret_Msg='Please Register'
	else
	begin
		select @custid=customer_id from customer where customer_first_name=@customer_fname and customer_last_name=@customer_lname and Phone=@customer_mobile;
		select @prodcatid=category_id from product_category where category_name=@product_category_name;
		select @prodid=product_id from product where product_name=@product_name and product_category_id=@prodcatid;
